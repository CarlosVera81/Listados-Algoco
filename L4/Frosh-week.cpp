...
int main() {
  ...
  minheap2.buildHeap(lista2); // O(m)

 
  while(true){
    if (minheap1.peek()==-1 || minheap2.peek()==-1){ // O(1)
      break;
    } else {
      if (minheap1.peek()<=minheap2.peek()){ // O(1)
        r++;
        minheap1.extractMin(); // O(log(n))
        minheap2.extractMin(); // O(log(m))
      }
      else {
        minheap2.extractMin(); // O(log(m))
      }
